...
class Geolocation:
	...
	def set_geolocation_information(self, tweet):
		try:
			...
			if tweet.geo:
				...
				if location in list(self.toplocations.keys()):
	...
	def show_top_locations(self, info_color, header_color, top, fun_color):
		try:
			...
			sort_loc = OrderedDict(sorted(list(self.toplocations.items()), key=itemgetter(1), reverse=True))
			loc = list(sort_loc.items())
...
class Hashtags:
	...
	def show_hashtags_information(self, info_color, header_color, fun_color):
		try:
			...
			sort_has = OrderedDict(sorted(list(self.hashtags_top.items()), key=itemgetter(1), reverse=True))
			self.hashtags_top = list(sort_has.items())[0:10]
...
class Mentions:
	...
	def show_mentions_information(self, info_color, header_color, fun_color):
		try:
			...
			sort_men = OrderedDict(sorted(list(self.mentions_top.items()), key=itemgetter(1), reverse=True))
			self.mentions_top = list(sort_men.items())[0:10]
				...
				for m in medias :
					...
					if str(media_url).find("video") >= 0:
						...
						if self.images == "d" or self.meta:
							...
							img = urllib.request.urlopen(media_url).read()
			...
			for p in self.pic:
				...
				if os.path.exists(path):
					...
					if fileExtension in (".jpg", ".jpeg"):			
						if img._getexif():
							metadata = 1
							exif = { ExifTags.TAGS[k]: v for k, v in list(img._getexif().items()) if k in ExifTags.TAGS }
							self.meta_description[p[0]] = str(exif['ImageDescription'])
							self.meta_copyright[p[0]] = str(exif['Copyright'])
							self.meta_date[p[0]] = str(exif['ImageDescription'])
							self.meta_make[p[0]] = str(exif['Make'])
							self.meta_model[p[0]] = str(exif['Model'])
							self.meta_software[p[0]] = str(exif['Software'])
							self.meta_distance[p[0]] = str(exif['SubjectDistance'][0]/float(exif['SubjectDistance'][1])) + " meters"
					...
					if 'icc_profile' in img.info:
						...
						if icc_profile:
							...
							self.meta_iccdate[p[0]] = str(time.strftime('%Y/%m/%d %H:%M:%S', final_tuple))
			...
			if os.path.exists(path):			
				...
				if fileExtension in (".jpg", ".jpeg"):			
					if img._getexif():
						metadata = 1
						exif = { ExifTags.TAGS[k]: v for k, v in list(img._getexif().items()) if k in ExifTags.TAGS }
						print_info(info_color, "Description: \t\t\t" + str(exif['ImageDescription']))
						print_info(info_color, "Copyright: \t\t\t" + str(exif['Copyright']))
						print_info(info_color, "Date: \t\t\t\t" + str(exif['DateTimeOriginal']))
						print_info(info_color, "Make: \t\t\t\t" + str(exif['Make']))
						print_info(info_color, "Model: \t\t\t\t" + str(exif['Model']))
						print_info(info_color, "Software: \t\t\t" + str(exif['Software']))
						print_info(info_color, "Subject distance: \t\t" + str(exif['SubjectDistance'][0]/float(exif['SubjectDistance'][1])) + " meters")
					
						if desc == "Profile Image":
							self.meta_profile_image.append(str(exif['ImageDescription']))
							self.meta_profile_image.append(str(exif['Copyright']))
							self.meta_profile_image.append(str(exif['DateTimeOriginal']))
							self.meta_profile_image.append(str(exif['Make']))
							self.meta_profile_image.append(str(exif['Model']))
							self.meta_profile_image.append(str(exif['Software']))
							self.meta_profile_image.append(str(exif['SubjectDistance'][0]/float(exif['SubjectDistance'][1])) + " meters")
...
def get_information(args, parameters):
	...
	try:
		...
		if not args.information and not args.friendname and not args.file and not args.number and not args.d and not args.sources and not args.hashtags and not args.mentions and not args.text and not args.meta:
			...
			#if args.output:
			return generates_HTML_file(parameters, user, source, hashtag, mentions, geolocation, user_images, user_tweets)
